...
const MarksPage = () => {
  ...
  // Show snackbar for responses
  useEffect(() => {
    if (response) {
      setSnackbar({ open: true, message: response, severity: "success" });
      dispatch(fetchMarksByTeacher(currentUser._id)); // Refresh marks list
    }
    if (error) {
  ...
  const handleUpdateSubmit = () => {
    ...
    dispatch(updateMarks(editingMark._id, formData));
  ...
  const handleDeleteConfirm = () => {
    dispatch(deleteMarks(markToDelete._id));
          ...
          <CardContent>
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 3 }}>
              <Button 
                ...
                disabled={!formData.studentId || !formData.subjectId}
                sx={{
